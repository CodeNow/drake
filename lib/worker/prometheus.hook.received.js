...
module.exports.task = (job) => {
  const eventData = job.commonLabels

  const log = logger.child({ eventData, module: 'datadogHook' })
  log.info('called')
  ...
  return Promise.try(() => {
    ...
    throw new WorkerStopError('event type not handled', { eventData })
  })
  .then((eventName) => {
    eventData.host = `http://${eventData.hostIp}:4242`
