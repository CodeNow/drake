...
/**
 * Determines if the event is a pull request.
 * @param {String} eventName Name of the github event.
 * @param {Object} payload Payload object delivered by GH
 * @param {String} payload.action Type of action take on that pull request
 * @return {Boolean} `true` if the event is a push.
 */
function isPullRequestOpenedEvent (eventName, payload) {
  return /^pull_request$/.test(eventName) && payload.action === 'opened'
}
...
module.exports = function githubHandler (request, reply) {
  ...
  return Promise
    ...
    .spread((eventType, deliveryId) => {
      ...
      if (isPullRequestOpenedEvent(eventType, request.payload)) {
        log.trace('Enqueuing Pull Request Opened Job')
        rabbitmq.publishEvent('github.pull-request.opened', {
          deliveryId: deliveryId,
          payload: request.payload
        })
        return reply('success').code(201)
      ...
      if (isCreateBranch(eventType)) {
        log.debug({ deliveryId: deliveryId }, 'Enqueuing create branch job')
        rabbitmq.publishEvent('github.branch.created', {
          deliveryId: deliveryId,
          payload: request.payload
        })
        return reply('success').code(201)
      ...
      if (isDeleteBranch(eventType)) {
        log.debug({ deliveryId: deliveryId }, 'Enqueuing delete branch job')
        rabbitmq.publishEvent('github.branch.deleted', {
          deliveryId: deliveryId,
          payload: request.payload
        })
        return reply('success').code(201)
      ...
      if (isPullRequest(eventType)) {
        log.debug({ deliveryId: deliveryId }, 'Enqueuing pull request kob')
        rabbitmq.publishEvent('github.pull_request.event', {
          deliveryId: deliveryId,
          payload: request.payload
        })
        return reply('success').code(201)
