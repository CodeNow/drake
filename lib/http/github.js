  ...
  return Promise
    ...
    .spread((eventType, deliveryId) => {
      ...
      if (isCreateBranch(eventType)) {
        ...
        log.debug({ deliveryId: deliveryId }, `Enqueuing create ${request.payload.ref_type} job`)
      ...
      if (isDeleteBranch(eventType)) {
        ...
        log.debug({ deliveryId: deliveryId }, `Enqueuing delete ${request.payload.ref_type} job`)
        rabbitmq.publishEvent(eventName, {
          deliveryId: deliveryId,
          payload: request.payload
        })
      ...
      if (isPullRequest(eventType)) {
        ...
        switch (request.payload.action) {
          ...
          case 'closed':
          case 'reopened':
            eventName = `github.pull-request.${request.payload.action}`
            break
          default:
            return reply('success').code(201)
        }
        log.debug({ deliveryId: deliveryId }, `Enqueuing pull request ${request.payload.ref_type} job`)
        rabbitmq.publishEvent(eventName, {
          deliveryId: deliveryId,
          payload: request.payload
        })
